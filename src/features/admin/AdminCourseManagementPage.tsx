...
const LessonForm: React.FC<LessonFormProps> = ({ courseId, sections, lesson, onSaved }) => {
  ...
  return <form className="flex flex-col gap-4" onSubmit={(event) => { event.preventDefault(); if (!title.trim()) { setError('Lesson title is required'); return; } if (!Number.isFinite(Number(estimatedTime)) || Number(estimatedTime) < 0) { setError('Estimated time must be zero or greater'); return; } setError(''); save.mutate(); }}>
